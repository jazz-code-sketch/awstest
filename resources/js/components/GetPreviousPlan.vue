    ...
    <v-dialog v-model="dialog" persistent max-width="400">
      <v-card class="elevation-6">
        <v-toolbar color="primary" dark flat>
          <v-toolbar-title class="arial">Base Plan</v-toolbar-title>
        </v-toolbar>
        <v-card-text class="pt-4" >
          ...
          <div class="pb-7">
            ...
            <!-- if have previous plan, show a listbox otherwise show textbox to input plan revision number -->
        ...
        <v-card-actions>
            <!-- <v-btn class="elevation-6" color="green darken-1" text @click="checkPlanDetails()" type="submit">新規登録</v-btn>
            <v-btn class="elevation-6" color="green darken-1" text @click="checkPlanDetails2()" type="submit">呼び出し</v-btn> -->
...
import { bus } from '../app'
  ...
  export default {
    ...
    computed: {
    //...mapState(['constructionCode', 'planNumber', 'houseType','requestNumber'])
    },
    methods:{
      selButton (){
        bus.$emit('changeIt', '');
      ...
      checkPlanDetails(){
        ...
        if (this.constructionCode !== '' && this.planNumber !== '' && this.houseType !== ''){      
          axios.get(`${window.location.origin}/api/getAllUploadedTextures/${this.constructionCode}${this.planNumber}${this.requestNumber}${this.houseType}`).then(res =>{ 
